...
import getpass
import coindre_optim.import_COOPT as imp
...
if __name__ == "__main__":
    ...
    print(
        "* Production plan is equal to the nominated/realised up to H{} included".format(
            lock_prod_plan_until_H
        )
    )
    ...
    gams_run_options = ' --MIP_GAP={} --RUN_NUM={} --COMMON_DIRECTORY="{}" --CURRENT_TIME={} --WARM_UP={} --REFINED_HYDRAULICS={} --FILTER_UC_SCHEDULE={} --LOCK_PRODUCTION_PLAN={} --RUN_TIME_LIMIT={} --INFLOWS_FACT_GR={} --INFLOWS_FACT_PR={}'.format(
        mip_gap,
        run_num,
        common_dir,
        time,
    ...
    print(
        "... Importing data from HDX ... ... Importing data from HDX ... ... Importing data from HDX ... "
    )
    ...
    print(
        "... LAUNCHING COOPT MODEL ... LAUNCHING COOPT MODEL ... LAUNCHING COOPT MODEL ... LAUNCHING COOPT MODEL ..."
    )
    os.system(cmd_run_launch)

    # post results in hdx parameters
    if post_directly_in_HDX == True:
        pr.post_results2hdx(common_dir=common_dir)
        print(
