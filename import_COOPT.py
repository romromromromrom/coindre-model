...
def write_model_inputs(PERSONAL_API_KEY,start_date,end_date, common_directory , write_csv = False,plot_brut = True,plot_imbal_pr = False,plot_imbal_gr = False,moving_averages = False,plot_scatters=False):

    c = psc.Client(bypass_proxy=True)
    c_hydro = hx.Client(api_key=PERSONAL_API_KEY )

    I_gr_proposed = c_hydro.get_node_calculated_timeseries(target_name='Grande-Rhue_R', param='inflow', start_date=start_date, end_date=end_date, scenario='proposed_forecast').rename(columns={'value':'igr'})
    I_pr_proposed = c_hydro.get_node_calculated_timeseries(target_name='Petite-Rhue_R', param='inflow', start_date=start_date,end_date=end_date, scenario='proposed_forecast').rename(columns={'value': 'ipr'})
    drange = pd.date_range(start=start_date,end=dt.datetime(I_gr_proposed.index[-1].year, I_gr_proposed.index[-1].month,I_gr_proposed.index[-1].day) + dt.timedelta(days=1) - dt.timedelta(hours=1), freq='1h', tz='Europe/Brussels')
    ...
    I_gr_q50 = c_hydro.get_node_calculated_timeseries(target_name='Grande-Rhue_R', param='inflow',start_date=start_date, end_date=end_date,scenario='proposed_forecast_q50').rename(columns={'value': 'igr_q50'})
    I_pr_q50 = c_hydro.get_node_calculated_timeseries(target_name='Petite-Rhue_R', param='inflow',start_date=start_date, end_date=end_date,scenario='proposed_forecast_q50').rename(columns={'value': 'ipr_q50'})
    ...
    valve_position = c_hydro.get_parameter_timeseries_data(target='Petite-Rhue_R', parameter='valve_position', scenario='productionplan', start_date=start_date, end_date=end_date)
    valve_position = valve_position.rename(columns={'value':'valve'}).resample('1h').pad()

    #import des débits
    tag = "FRCOINDR_2_____FLOW________MMB"
    resp = c.get_pi_summary_data(start_date, end_date, tag=tag,step='1h',type='Average')
    ...
    z_gr_norm = pd.DataFrame(z_gr.loc[:,'zgr'] - 686.28).rename(columns={'zgr':'zgr_norm'})
    ...
    z_pr_norm = pd.DataFrame(z_pr.loc[:,'zpr'] - 684.7).rename(columns={'zpr':'zpr_norm'})
    ...
    spot1 = c_hydro.get_node_calculated_timeseries(target_name='Spot', param='spot_price', start_date=start_date, end_date=end_date, scenario='price_phoenix').rename(columns={'value':'spot1'})
    spot2 = c_hydro.get_node_calculated_timeseries(target_name='Spot', param='spot_price', start_date=start_date, end_date=end_date, scenario='price_epexspot').rename(columns={'value':'spot2'})
    spot3 = c_hydro.get_node_calculated_timeseries(target_name='Spot', param='spot_price', start_date=start_date,end_date=end_date, scenario='price_41740').rename(columns={'value': 'spot3'})
    merged_df = pd.concat([spot1,spot2], axis=1)
    merged_df.loc[merged_df['spot2'].isna().values,'spot2'] = merged_df[merged_df['spot2'].isna().values].loc[:,'spot1'].values
    spot_merged = pd.DataFrame(merged_df.loc[:,'spot2']).rename(columns={'spot2':'spot_merged'})
    merged_df2 = pd.concat([spot_merged,spot3], axis=1)
    merged_df2.loc[merged_df2['spot_merged'].isna().values, 'spot_merged'] = merged_df2[merged_df2['spot_merged'].isna().values].loc[:,'spot3'].values
    ...
    P = c_hydro.get_node_calculated_timeseries(target_name='Coindre_T', param='production', start_date=start_date, end_date=end_date, scenario='productionplan').rename(columns={'value':'MW'})
    # Import des unavails
    scen_unavail = 'Default'
    unavail= c_hydro.get_node_calculated_timeseries(target_name='Coindre_T', param='unavailability', start_date= start_date, end_date= end_date, scenario=scen_unavail).rename(columns={'value':'unavail'})
    ...
    spill_pr2 = spill_pr.tz_convert('Europe/Brussels').rename(columns={'Value':'spill_pr'})
    spill_gr2 = spill_gr.tz_convert('Europe/Brussels').rename(columns={'Value':'spill_gr'})

    # computation of the water imbalance in the system taking into account the spilled waters
    vpr_t_plus_1 = pd.DataFrame(data=np.roll(v_pr, -1), index=v_pr['vpr'].index, columns=['Vt+1'])
    WB_PR =    -(  (-I_pr['ipr'] + q_pr['qpr'] + spill_pr['spill_pr'] ) + (vpr_t_plus_1['Vt+1'] - v_pr['vpr']) /3600)
    WB_PR = pd.DataFrame(data=WB_PR, columns=['wb_pr'])
    vgr_t_plus_1 = pd.DataFrame(data=np.roll(v_gr, -1), index=v_gr['vgr'].index, columns=['Vt+1'])
    WB_GR = -( (-I_gr['igr'] + q_gr['qgr'] + spill_gr['spill_gr'] )  + (vgr_t_plus_1['Vt+1'] - v_gr['vgr'])/3600)
    ...
    df = pd.concat([ spill_gr2, spill_pr2, wb_gr2, wb_pr2, qpr, qgr , igr2, ipr2, p2, valve2, spot['spot'], unavail['unavail'], zpr2, zgr2, dz2, v_gr,v_pr],axis=1).fillna(method='ffill')
    ...
    if write_csv:
        ...
        igr2.to_csv(path_or_buf= common_directory+r'\input\inflows_gr.csv', header=True)
        ipr2.to_csv(path_or_buf= common_directory+r'\input\inflows_pr.csv', header=True)
        ...
        spill_gr2.to_csv(path_or_buf=common_directory+'\input\spill_gr.csv', header=True)
        spill_pr2.to_csv(path_or_buf=common_directory+r'\input\spill_pr.csv', header=True)

        vane.to_csv(path_or_buf= common_directory+r'\input\vane_closed.csv', header=True)
        spot.to_csv(path_or_buf= common_directory+r'\input\spot.csv', header=True)
        unavail.to_csv(path_or_buf= common_directory+r'\input\unavail.csv', header=True)
